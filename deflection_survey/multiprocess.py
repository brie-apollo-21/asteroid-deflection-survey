...
def pool_func(ra, dec, magnitude, epoch, asteroid):
    # print(asteroid['name'])
    # Reset CSV
...
def survey(asteroid, angle_resolution, epoch_resolution, magnitude):
    ...
    iter = [(ra,dec,magnitude,epoch,asteroid) for ra in range(0, 360, angle_resolution) for dec in range(0, 360, angle_resolution) for epoch in epochs]
    # print(len(iter))
...
def main():
    ...
    for asteroid in asteroids:
        ...
        # f.write("TDB_Gregorian,TDB_MJD,Magnitude,RA,DEC,Earth_RMAG,Earth_Altitude")
        # f.close()
        survey(asteroid, angle_resolution, epoch_resolution, magnitude)
